...
Comparing the estimated deviance in Kaitagskii district between 2012 and 2018 (the presidential elections before and after the sentencing of the precinct chair) shows that precinct-level deviance for last-digit zero declined considerably: from 15.11 to 3.47. This reduction in estimated fraud, a decline of 13.8, is the third largest in Dagestan. In fact, the average change by district is an *increase* in deviance of 0.48. Three of the four rural regions that adjoin Kaitagskii district also saw relative declines in deviance. An urban district, Derbentskii, saw a large increase in deviance. 

Figure \@ref(fig:fig-map) shows a map of Dagestan with its administrative divisions. The districts on the map have been divided into three bins based on the size and direction of the change in last-digit deviance between the two presidential elections. The variable has been centered and scaled for easier visualization. Large negative changes (the bottom quartile) have been coded in light red; large positive changes (the upper quartile) have been coded in dark blue. Changes in the middle of the distribution, close to zero, have been coded in light blue. Kaytagskii district, where the court case was first heard, has been marked with a solid triangle. As the map illustrates, some of the largest declines in estimated fraud occur in neighboring rural, mountainous districts. The neighboring dark blue regions, where fraud seemingly increased substantially, are qualitatively different\textemdash coastal, more urban, and home to different ethnic groups. This is consistent with a version of events in which a 'signal' about the cost of committing fraud is received by front-line election workers within a similar milieu and similar social networks as the defendant in the court case.
...
sf <- st_read(dsn= here("Maps", "RUS_adm"), layer="RUS_adm2") %>% dplyr::filter(NAME_1 == "Dagestan")
shape <- readOGR(dsn=here("Maps", "RUS_adm"), layer="RUS_adm2")  
shape <- subset(shape, NAME_1 == "Dagestan")

## Read in deviance diffs and merge with sf

deviance_diffs <- read.csv(here("Data", "dagestan_deviance_diffs_mapcodes.csv")) %>% dplyr::select(ID_2, deviance_diff)
